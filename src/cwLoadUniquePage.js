...
(function (cwApi, $) {
  ...
  function getPageNavigationForSinglePage(item, objectTypeScriptName) {
    ...
    viewsLoaded = cwApi.sortBy(viewsLoaded, function (v) {
      if (cwApi.isUndefined(v)) {
        cwApi.Log.Error(
          "One or several views are not correct part of your views, please check case of the views to don't have duplicates, Please check part of these views : " +
            views.join(",")
        );
        return 0;
      }
    ...
    for (i = 0; i < viewsLoaded.length; i += 1) {
      ...
      if (!cwApi.isUndefined(v) && cwApi.cwUser.canAccessView(cwApi.currentUser, v) === true) {
        ...
        if (
          cwApi.isQuestionnaireBehaviuor(viewSchema.Behaviours) &&
          objectTypeScriptName.toLowerCase() !== cwApi.mmDefinition.OBJECTTYPE_SCRIPTNAME_QUESTIONNAIRE
        ) {
          questionnaireQueTab = {
            id: "tab" + tabCount,
  ...
  cwApi.loadObjectPage = function (view, objectId, rootNodeSchema, callback) {
    ...
    var jsonFile,
      ...
      discussionData,
      isProfile,
      allowValidationRequestForScriptNames,
      allowValidationRequestForThisObject,
      objectIsUnderValidation,
      isQuestionnaireView,
    ...
    cwApi.getJSONFile(
      ...
      function (json) {
        ...
        if (cwApi.checkJsonCallback(json)) {
          ...
          if (!cwApi.isLive() || jsonFile.indexOf("/statics/") > 0 || jsonFile.indexOf("/live/") > 0) {
            object = json;
          } else {
            object = json.object;
          }
